...
func TestMetricSamplesReadLabelsIndependentlyOfOrder(t *testing.T) {
	t.Parallel()
...
func TestMetricSamplesSumExplicitly(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(
			"sip_exporter_rtp_packets_total{direction=\"inbound\"} 3\n" +
				"sip_exporter_rtp_packets_total{direction=\"outbound\"} 4\n",
		))
	}))
	t.Cleanup(server.Close)

	require.Equal(t, 7.0, getMetricSum(t, server.URL, "sip_exporter_rtp_packets_total"))
}
