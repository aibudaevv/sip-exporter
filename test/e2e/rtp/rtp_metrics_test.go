...
// metricLineExists keeps the historical helper name while using exact metric
// parsing rather than matching label text in the Prometheus exposition.
func metricLineExists(t *testing.T, endpoint, name string, labels ...string) bool {
	t.Helper()
	return metricWithLabelsExists(t, endpoint, name, labels...)
}
