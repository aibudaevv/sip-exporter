...
В этой статье — как я решил задачу мониторинга SIP-телефонии с помощью eBPF: от захвата пакетов в ядре Linux до метрик RFC 6076 с разбивкой по источникам трафика и типам устройств в Prometheus/VictoriaMetrics.
...
Carrier показывает **кто** звонит, но не показывает **чем**. А тип устройства часто является ключевым фактором проблем: IP-телефоны ломаются по-другому, чем софтфоны, а прошивки Yealink и Grandstream имеют разные баги.
...
Как и carrier, ua_type определяется в момент запроса и наследуется ответами через трекер по Call-ID. Ответы SIP обычно содержат заголовок `Server`, а не `User-Agent` — поэтому классификация по запросу — единственный надёжный способ.
