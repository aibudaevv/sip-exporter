...
**Приватность RTP:** Приложение читает только 12-байный RTP-заголовок (версия, PT, sequence, timestamp, SSRC). Голосовой payload недоступен приложению. RTP-метрики содержат только лейблы `carrier`, `ua_type` и `codec` — никаких телефонных номеров, IP-адресов или идентификаторов звонков.
