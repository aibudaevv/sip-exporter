...
- **Не** сохраняет данные пакетов в файлах, которыми управляет приложение. Production Compose-пример пишет только анонимный telemetry-ID в volume `sip-exporter-state`; конфигурационные mounts подключаются как `:ro` (только чтение). Логирование контейнера может сохранять чувствительные диагностические данные; см. [Логирование и чувствительные идентификаторы](#логирование-и-чувствительные-идентификаторы)
...
## Логирование и чувствительные идентификаторы

Уровень `info` по умолчанию не пишет raw payload пакетов, но предупреждения FAS содержат SIP Call-ID. На уровне `debug` диагностические записи содержат raw SIP-сообщения и могут включать Call-ID, IP-адреса и порты media endpoints. Эти записи идут в stdout/stderr, а не в volume `sip-exporter-state`, однако logging driver Docker или оркестратора может их сохранять.

В production используйте `SIP_EXPORTER_LOGGER_LEVEL=info` (по умолчанию) или `error`, ограничьте доступ к собранным логам и настройте подходящий срок хранения. Включайте `debug` только на контролируемое время диагностики. Допустимые значения: `error`, `info` и `debug`; любое другое значение сейчас включает debug-логирование.
