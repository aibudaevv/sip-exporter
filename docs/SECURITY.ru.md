...
Весь eBPF-фильтр — это [~130 строк на C](../internal/bpf/sip.c). Он делает две вещи: (1) пропускает UDP-трафик на настроенном SIP-порту (по умолчанию 5060), и (2) пропускает RTP-пакеты от media-endpoint'ов, извлечённых из SDP-сигнализации (INVITE 200 OK).
