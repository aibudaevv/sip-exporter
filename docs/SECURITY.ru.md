...
**Приватность RTP:** приложение разбирает только фиксированный 12-байтовый RTP-заголовок (версия, PT, sequence, timestamp, SSRC), но 64-байтовый снимок захвата может включать небольшой префикс payload. Аудио не инспектируется и не сохраняется. RTCP compound-пакеты копируются до 1518 байт для разбора report blocks. RTP-метрики содержат только лейблы `carrier`, `ua_type`, `codec`, `source_country` и `direction` — номера телефонов, raw IP и идентификаторы вызовов в RTP-метрики не попадают. Полный перечень лейблов всех семейств метрик приведён в [Данные в лейблах Prometheus](#данные-в-лейблах-prometheus).
