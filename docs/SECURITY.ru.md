...
> **Особенность мульти-интерфейса:** eBPF socket filter общий для всех настроенных интерфейсов. Одна BPF-коллекция загружается один раз при старте; та же программа подключается к N сокетам `AF_PACKET` через `SO_ATTACH_BPF` (по сокету на интерфейс). Maps `sip_ports` и `rtp_config` глобальные — одинаковые правила фильтрации на всех интерфейсах.
