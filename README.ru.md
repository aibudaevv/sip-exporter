...
Опубликованные таблицы бенчмарков — исторические наблюдения, а не актуальный контракт проверки
релиза. Показатели пропускной способности, sizing, задержки scrape и multi-interface сейчас проходят
повторную проверку со строгим контролем генератора, целостности захвата, ресурсов и baseline. До её
завершения исторические цифры нельзя использовать для приёмки релиза или production sizing.
...
Исторические результаты и их текущий статус проверки: [docs/BENCHMARK.md](./docs/BENCHMARK.md).
...
Опубликованные результаты нагрузочного тестирования являются историческими, сейчас не подтверждены
release verification и не подходят для production sizing.

Исторические результаты и их статус проверки приведены в
